...
class VariantSelects extends HTMLElement {
  ...
  renderProductInfo() {
    ...
    fetch(
      ...
      .then((responseText) => {
        ...
        // tom:
        // get name of add to cart button
        // support pre-order case
        const addToCartText = html.querySelector('.product-form__submit').textContent.trim();

        /** @type {HTMLDivElement} */
        const shopifyBuyBtn = document.querySelector(`[data-shopify="payment-button"]`);

        switch (addToCartText) {
          case 'Pre-order':
            this.submitBtnText = 'Pre-order';
            if (shopifyBuyBtn) {
              shopifyBuyBtn.style.opacity = '0';
              shopifyBuyBtn.style.pointerEvents = 'none';
            }
            break;
          default:
            this.submitBtnText = void 0;
            if (shopifyBuyBtn) {
              shopifyBuyBtn.style.opacity = '100';
              shopifyBuyBtn.style.pointerEvents = 'auto';
            }
        }
        ...
        document.querySelector('variant-radios')
          ? this.querySelector(`[for="${activeElementId}"]`).focus()
          : this.querySelector(`#${activeElementId}`).focus();
      });
  }
  ...
  toggleAddButton(disable = true, text, modifyClass = true) {
    ...
    if (disable) {
      ...
      // tom:
      // support pre-order case and other cases in future.
      if (this.submitBtnText) addButtonText.textContent = this.submitBtnText;
      else addButtonText.textContent = window.variantStrings.addToCart;
    }
